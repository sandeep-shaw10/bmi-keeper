...
const BMICalculator = () => {
  ...
  const {heightUnit, weightUnit, t} = useContext(AppStateContext)
  const OUTPUT = BMICalculation(age, sex, weight, height, heightUnit.conversion, weightUnit.conversion)

  return (
    <ScrollView contentInsetAdjustmentBehavior="automatic">
    <View style={styles.container}>
      <AgeSexInput title={t('Age')} age={age} setAge={setAge} sex={sex} setSex={setSex} />
      <CombinedUnitInput unitType='height' title={t('Height')} value={height} setValue={setHeight} />
      <CombinedUnitInput unitType='weight' title={t('Weight')} value={weight} setValue={setWeight} />
    </View>
    <View>
      {
        isNaN(age) ? null :
